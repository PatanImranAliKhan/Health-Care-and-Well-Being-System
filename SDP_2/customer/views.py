...
def ProfilePage(request):
    ...
    if request.method=="POST" and "update" in request.POST:
        ...
        if(request.POST['oldpass']!="" and request.POST['oldpass']!=pat.password):
            return render(request,'cusprofile.html',{'patient':pat,'errorp':'incorrect password'})
        if(request.POST['newpass']!="" and request.POST['newpass']!=request.POST['confirmpass']):
            return render(request,'cusprofile.html',{'patient':pat,'errorcon':'password wasnot matched'})
        ...
        pat=Patient.objects.get(email=request.session['email'])
        return render(request,'cusprofile.html',{'patient':pat,'success':'successfully updated the data'})
    return render(request,'cusprofile.html',{'patient':pat})